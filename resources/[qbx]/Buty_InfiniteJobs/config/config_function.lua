...
if Configuration.UseFuel then
    function fuel(fuelvehicle)
        Entity(fuelvehicle).state.fuel = 100
    end
end
...
function progress()
    if Configuration.framework == 'esx' then
        exports['BCall_progress']:muestra(4020, 'Sagatavojās...')
    else
        exports['BCall_progress']:muestra(4020, 'Pārvelk drēbes…')
    end
end
...
function Blips(NPC)
    for i, v in ipairs(NPC) do
        ...
        AddTextComponentSubstringPlayerName("Dome")
