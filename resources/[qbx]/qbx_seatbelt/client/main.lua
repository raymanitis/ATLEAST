...
local function toggleSeatbelt()
    ...
    -- Trigger the HUD export for seatbelt toggle
    if seatbeltOn then
        exports['east_hud']:seatbelt(true)  -- Turn HUD seatbelt indicator green (on)
    else
        exports['east_hud']:seatbelt(false) -- Turn HUD seatbelt indicator red (off)
    end

    -- Set fly-through windscreen parameters based on seatbelt status
    SetFlyThroughWindscreenParams(seatbeltOn and minSpeeds.buckled or minSpeeds.unbuckled, 1.0, 17.0, 10.0)

    -- Trigger the seatbelt toggle event and play the appropriate sound
