...
-- Light Armor (gives up to 50 armor)
Item('lightarmor', function(data, slot)
    ...
    if currentArmor < 50 then
        ox_inventory:useItem(data, function(data)
            if data then
                local newArmor = math.min(50, currentArmor + (50 - currentArmor))
                SetPlayerMaxArmour(PlayerId(), 50)
                SetPedArmour(cache.ped, newArmor)
                exports.qbx_core:Notify("Vieglā bruņa uzlikta. Bruņas: " .. newArmor .. "/50", 'success', 5000)
            end
        end)
    else
        exports.qbx_core:Notify("Tev jau ir pietiekami daudz bruņu.", 'inform', 5000)
    end
end)

-- Armor (gives up to 75 armor)
Item('armor', function(data, slot)
    local currentArmor = GetPedArmour(cache.ped)
    
    if currentArmor < 75 then
        ox_inventory:useItem(data, function(data)
            if data then
                local newArmor = math.min(75, currentArmor + (75 - currentArmor))
                SetPlayerMaxArmour(PlayerId(), 75)
                SetPedArmour(cache.ped, newArmor)
                exports.qbx_core:Notify("Bruņas uzliktas. Bruņas: " .. newArmor .. "/75", 'success', 5000)
            end
        end)
    else
        exports.qbx_core:Notify("Tev jau ir pietiekami daudz bruņu.", 'inform', 5000)
...
-- Heavy Armor (gives up to 100 armor)
    ...
    if currentArmor < 100 then
        ox_inventory:useItem(data, function(data)
            if data then
                ...
                exports.qbx_core:Notify("Smagās bruņas uzliktas. Bruņas: 100/100", 'success', 5000)
            end
        end)
    else
        exports.qbx_core:Notify("Tev jau ir pietiekami daudz bruņu.", 'inform', 5000)
